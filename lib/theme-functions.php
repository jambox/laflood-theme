...
function laflood_location_archive_query( $query ) {
  if ( is_location_archive() && $query->is_main_query() && !is_admin() ) {
    set_query_var('post_parent', 0 );
    set_query_var('post_status', array('publish') );
    set_query_var('orderby', 'menu_order' );
  }
} // END laflood_location_archive_query()

add_action('pre_get_posts','laflood_location_archive_query');


function laflood_add_location_category_archive( $query ) {
  if( is_category() && $query->is_main_query() ) {
    $post_type = get_query_var('post_type');
    $proj_cpt = get_location_cpt_name();
    $post_type = $post_type ? array($post_type, $proj_cpt) : array($proj_cpt);
    $query->set('post_type',$post_type);
  }
    return $query;   

} // END laflood_add_location_category_archive()

add_action('pre_get_posts','laflood_add_location_category_archive');


function laflood_only_allow_search_for_location( $query ) {
  if ($query->is_search) {
      $query->set( 'post_type', get_location_cpt_name() );
  };
  return $query;   

} // END laflood_add_location_category_archive()
add_action('pre_get_posts','laflood_only_allow_search_for_location');
